...
class RelativeNav:
    ...
    def marker_array_callback(self,marker_array):
        ...
        for i in range(len(marker_array.markers)):
            if  not (math.isnan(marker_array.markers[i].pose.position.x)):
                ...
                if (marker_array.markers[i].type == POLE and marker_array.markers[i].color.g == 1.0 ): # we want to find  a green pole
                    green_pole_flag = 1
                    if (closest_green_marker.pose.position.x == -1 or marker_array.markers[i].pose.position.x < closest_green_marker.pose.position.x):
